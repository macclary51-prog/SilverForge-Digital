...
Release checks on September 15, 2026 verified project **silverforge-digital**, project number **684696359962**, with billing disabled. The owner chose to **keep Spark**. The Cloud Functions and Secret Manager APIs returned `SERVICE_DISABLED`; existing remote resources could not be inventoried through those APIs. No billing change, function deployment or secret deletion was performed. No legacy notification functions or secrets had been deployed by the preceding implementation. If anything was added independently, inspect that project's function/secret inventory after enabling the required access and remove only obsolete notification resources. Do not remove unrelated functions or secrets.

PR #9 was merged as `a53f2b227167d9d46134502c67f4bcdc068c5b17` and successfully published through GitHub Pages. Thirteen production files, including the notification UI, worker, manifest, icons, contact and quote pages, matched the merged source. The Firestore rules were deployed only to the verified project and read back successfully: ruleset `68bc192f-91c2-4fe0-8bec-23b7bb83838c`, released `2026-09-16T01:59:29Z` (September 15 Pacific time). The FCM HTTP v1 and FCM Registration APIs are both enabled. A new Web Push key pair was generated because none existed; its public key was saved through the live admin Notification Settings page while preserving the category/channel selections. The live page reported the successful save and no browser console errors.

**Automatic push alerts and new server-created notification history are not active while the six sender functions remain undeployed.** The website, PWA and device-registration setup are published, but no admin devices were registered during release verification. Each admin must explicitly click **Enable Notifications** on their own device. Real token issuance, FCM acceptance, Android installation and operating-system delivery remain unverified; complete the setup and live checklist below when the backend is available. Automated token tests use an explicit mock because FCM has no emulator.
